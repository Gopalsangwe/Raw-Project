...
public class ServiceProviderController {
	...
	/*
	 * @GetMapping("/{spId}/bookings") public List<Booking>
	 * getBookingsForSp(@PathVariable Long spId) { return
	 * spService.getAllBookingsById(spId); }
	 */
	...
	public ServiceProvider loginAuthentication(@RequestBody SpLoginDto logindto) {
	 ...
	 @PostMapping("/{serviceProviderId}/categories/{categoryId}/vehicles/")
	    public ResponseEntity<Void> addVehiclesToServiceProvider(@PathVariable Long serviceProviderId, @PathVariable Long categoryId,@RequestBody VehicleRegisterationDto transientvehicledto) {
	        spService.addVehicleToServiceProvider(serviceProviderId, categoryId,transientvehicledto);
	        return ResponseEntity.ok().build();
	    }
